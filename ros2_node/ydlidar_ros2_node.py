...
class YDLidarRos2Node(Node):
    def __init__(self):
        ...
        # Single channel for many models; leave intensity enabled by default
        try:
        ...
        for b in baud_list:
            try:
                ...
                if not ok:
                    self.get_logger().info('turnOn() returned False for baud %d' % b)
                    # If initialize succeeded but turnOn failed, try next baudrate
                    try:
                        self.laser.turnOff()
                    except Exception:
                        pass
    ...
    def _read_loop(self):
        ...
        while rclpy.ok() and self._run_thread:
            try:
                ...
                # Fill ranges/intensities. Points are expected to be ordered by angle.
